 ...
 * Any memory allocated by LamKit.Malloc needs to be released with LamKit.Free.
 *
 * const mObj = LamKit.Malloc(Float32Array, 20);
 * Get a TypedArray view around the malloc'd memory (this does not copy anything).
 * const ta = mObj.toTypedArray();
 * // store data into ta
 * const cf = LamKit.ColorFilter.MakeMatrix(ta); // mObj could also be used.
 *
 * // eventually...
 * LamKit.Free(mObj);
 *
...
// that was returned by LamKit.Malloc.
...
var _scratch3x3Matrix;  // the result from LamKit.Malloc
...
// If color is an object that was allocated with LamKit.Malloc, its pointer is
...
function copyFlexibleColorArray(colors) {
  var result = {
    colorPtr: nullptr,
    count: colors.length,
    colorType: LamKit.ColorType.RGBA_F32,
  };
  if (colors instanceof Float32Array) {
    ...
    result.colorType = LamKit.ColorType.RGBA_8888;
...
function copyColorArray(arr) {
  ...
  var ptr = LamKit._malloc(arr.length * 4 * 4);

  var idx = 0;
  var adjustedPtr = ptr / 4; // cast the byte pointer into a float pointer.
  for (var r = 0; r < arr.length; r++) {
    for (var c = 0; c < 4; c++) {
      LamKit.HEAPF32[adjustedPtr + idx] = arr[r][c];
      idx++;
    }
  }
  return ptr;
...
// Copies the given DOMMatrix/Array/TypedArray to the LamKit heap and
  ...
  if (matr.length) {
    if (matr.length === 6 || matr.length === 9) {
      ...
      if (matr.length === 6) {
        ...
        LamKit.HEAPF32.set(defaultPerspective, 6 + _scratch3x3MatrixPtr / 4);
      }
...
function copy4x4MatrixFromWasm(matrPtr) {
  ...
  var rv = new Array(16);
  for (var i = 0; i < 16; i++) {
    rv[i] = LamKit.HEAPF32[matrPtr/4 + i]; // divide by 4 to cast to float.
  }
  return rv;
}
...
function copyColorFromWasm(colorPtr) {
  ...
  for (var i = 0; i < 4; i++) {
    rv[i] = LamKit.HEAPF32[colorPtr/4 + i]; // divide by 4 to cast to float.
  }
  return rv;
}
